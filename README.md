...
## 입출력 절차
1. **구입 금액 입력**: 숫자가 아니거나 1,000원 단위가 아니면 `[ERROR]` 메시지를 출력하고 다시 입력받는다.
2. **로또 발행 결과 출력**: 구매한 장수와 각 로또 번호(오름차순)를 출력한다.
3. **당첨 번호 입력**: 쉼표로 구분된 6개 숫자를 입력받고 숫자·범위·중복 조건을 검증한다.
4. **보너스 번호 입력**: 단일 숫자를 입력받고 범위 및 당첨 번호 중복 여부를 검증한다.
5. **당첨 통계 및 수익률 출력**: 등수별 개수와 총 수익률을 출력한다.
...
## 예외 처리
- 잘못된 입력이 들어오면 `IllegalArgumentException`을 발생시키고 `[ERROR]` 메시지를 출력한 뒤, 해당 단계부터 다시 입력받는다.
- 내부 상태가 잘못되면 `IllegalStateException`으로 구분한다.
- 주요 검증 항목
...
## 테스트
- `src/test/java/lotto/model/**`에서 입력 파서, 검증기, 서비스, 수익 계산 로직을 단위 테스트한다.
- `src/test/java/lotto/model/domain/**`에서 로또 및 당첨 도메인 객체 동작을 검증한다.
- `src/test/java/lotto/ApplicationTest`에서 통합 시나리오와 예외 흐름을 점검한다.
